...
# Author:jiang
# 2021/5/19 15:57
import wx,MySQLdb,threading,time,datetime
...
class DataInitialization(wx.Panel):
    ...
    def __init__(self, parent):
        ...
        self.btn1 = wx.Button(self, label="确认", size=(85, 25), pos=(670, 2))
        self.btn2 = wx.Button(self, label="停止", size=(85, 25), pos=(760, 2))
        self.btn1.Bind(wx.EVT_BUTTON, self.startFuntion)
        self.btn2.Bind(wx.EVT_BUTTON, self.stopFunction)
        hander=MyLogHandler(self.result)
        logging.getLogger().addHandler(hander)

    # def OnTimerEvent(self, event): #点击停止,日志输出结束
    #     # logging.info(datetime.datetime.now())
    #     self.flushProduct("305006")
    #     self.perfectProductInfomation("305006")
    def stopFunction(self,event):
        self.btn2.Disable()
        self.btn1.Enable()
    def startFuntion(self,event):
